...
broad level view of memory use. 
'''
...
print("""
      ...
      profile_limt=100 gunicorn -c ./wsgi_profiler.py app:server

      To filter for a particular method and limit printout:
      profile_limit=10 profile_method=methodName gunicorn -c ./wsgi_profiler.py app:server

      To filter for a particular file:
      profile_file=functions.py gunicorn -c ./wsgi_profiler.py app:server

      To restrict printouts to element times only:
      profiler=0 gunicorn -c ./wsgi_profiler.py app:server
""")
